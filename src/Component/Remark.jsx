        ...
        <div className="section">
                    ...
                    <div className='grid grid-cols-5 gap-2 container'>
            {
                ListByRemark?.map((item,i)=>(
                    <NavLink to={`/blog-details/${item._id}`} key={i} className='h-60 w-52 bg-yellow-300 m-6 rounded-md no-underline text-black'>
                    <img className='h-2/4 w-52 rounded-t-md' src={item.image} alt="" />
                    <div  className='mt-4'>
                    <p className='m-1 ml-2'><span className='font-bold my-2'>Title: </span>{item.title}</p>
                    <div className='ml-2 flex'><span className='mt-1 mr-1'><GiCheckMark /></span ><span  className='font-bold'>Remark: </span> <span className=''>{item.remark}</span></div>
                        </div>
                    </NavLink>
                ))
            }
